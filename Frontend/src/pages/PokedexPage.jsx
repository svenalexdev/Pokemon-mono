      ...
      <div className="flex flex-1s h-[calc(100vh-4rem)]">
        <div className="w-1/2 p-4 overflow-y-auto border-r border-blue-300">
          {pokemonList.map((pokemon, index) => (
            <div
              key={index}
              className="flex items-center mb-4 bg-white rounded-lg shadow p-3 hover:bg-blue-100 transition"
            >
              <img
                src={pokemon.sprites.front_default}
                alt={pokemon.name}
                className=" w-25 h-25 mb-2 mr-4 object-contain mx-auto"
              />
              <div className="flex-1">
                <p className="text-xs text-gray-600"># {pokemon.id}</p>
                <p className="text-base font-semibold capitalize">{pokemon.name}</p>
                <button
                  onClick={() => setSelectedPokemon(pokemon)}
                  className="mt-1 text-xs bg-indigo-500 text-white px-2 py-1 rounded hover:bg-indigo-600"
                >
                  Detail
                </button>
              </div>
          ...
          {selectedPokemon ? (
            <div className="bg-white rounded-lg shadow-lg p-6">
              <div className="text-center">
                ...
                <p className="text-gray-600 text-lg mb-4"># {selectedPokemon.id}</p>
              </div>

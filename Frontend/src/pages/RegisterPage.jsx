...
function RegisterPage() {
  ...
  const handleSubmit = async e => {
    try {
      ...
      setLoading(true);
      const { message } = await testRegister({ username, email, password });
  ...
  const testRegister = ({ username, email }) => {
    return new Promise(res => setTimeout(() => res({ message: `Trainer ${username} registered with ${email}` }), 1000));
  };
